    ...
    for (y in 1:length(Years)){
      ...
      if (any(swathyear)){
        ...
        if (!pool.sides){
          ...
          Dfish$stipespt.sd[OKy] <- mean(Dswath$stipespt.sd[swathyear],na.rm=TRUE)
        } # end if pool.sides
        ...
        if (interp){
          for (d in 1:length(Sides)){
            Dfish$MACPYRAD[OKy & Dfish$Side==Sides[d]] <- median(Dswath$MACPYRAD[OKswath],na.rm=TRUE)
            Dfish$stipespt[OKy & Dfish$Side==Sides[d]] <- median(Dswath$stipespt[OKswath] ,na.rm=TRUE)
            Dfish$stipespt.sd[OKy & Dfish$Side==Sides[d]] <- median(Dswath$stipespt.sd[OKswath] ,na.rm=TRUE)
            }}}
